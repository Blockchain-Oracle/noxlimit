...
/// @notice Market-bound private-limit order book for one curated binary FPMM.
/// @dev The contract enforces the order lifecycle and asset-safety invariants; deployment-specific
///      market resolution, catalog rotation, and production hardening live outside this contract.
contract NoxLimitOrderBook {
    ...
    constructor(
        ...
        if (
            evaluationTimeout_ == 0 ||
            publicationTimeout_ == 0 ||
            minimumEvaluationInterval_ == 0 ||
            maximumEvaluations_ == 0
        ) revert InvalidMonitoringPolicy();
        if (tradingClosesAt_ <= block.timestamp) revert MarketClosed();
        if (
    ...
    function createOrder(
        ...
        if (expiresAt <= block.timestamp || expiresAt > tradingClosesAt) revert InvalidExpiry();
        ...
        returns (uint256 plaintext)
    {
        ...
        _requireNonce(order, nonce);
        if (
            block.timestamp >= tradingClosesAt ||
            block.timestamp >= order.expiresAt ||
            block.timestamp >= order.phaseDeadline
        ) {
    ...
    function expireEvaluation(uint256 orderId) external {
        ...
        order.phaseDeadline = 0;
        if (block.timestamp >= order.expiresAt || block.timestamp >= tradingClosesAt) {
    ...
    function phaseDeadlineOf(uint256 orderId) external view returns (uint64) {
        return _orders[orderId].phaseDeadline;
    }

    function lastEvaluationAtOf(uint256 orderId) external view returns (uint64) {
        return _orders[orderId].lastEvaluationAt;
    }
    ...
    function evaluationCountOf(uint256 orderId) external view returns (uint8) {
        return _orders[orderId].evaluationCount;
    }

    function remainingEvaluations(uint256 orderId) external view returns (uint8) {
        Order storage order = _orders[orderId];
        if (order.status == Status.None) return 0;
        uint8 used = order.evaluationCount;
        return used >= maximumEvaluations ? 0 : maximumEvaluations - used;
    }

    function candidateOf(uint256 orderId) external view returns (bytes32) {
        Order storage order = _orders[orderId];
        if (
            (order.status != Status.Evaluating && order.status != Status.PublicationPending) ||
            block.timestamp >= tradingClosesAt ||
            block.timestamp >= order.expiresAt ||
            block.timestamp >= order.phaseDeadline
        ) return bytes32(0);
